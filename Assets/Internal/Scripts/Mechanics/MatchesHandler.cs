...
public class MatchesHandler : MonoBehaviour
{
    ...
    public HashSet<Item> FindMatches()
    {
        ...
        // Горизонтальные совпадения (только в активных ячейках)
        for (int y = 0; y < _height; y++)
        {
            for (int x = 0; x < _width - 2; x++)
            {
                if (!_board.IsActiveCell(x, y)) continue;
                    ...
                    _allItems[x + 2, y]._itemType == current._itemType)
                {
                    ...
                    {
                        if (_board.IsActiveCell(i, y) && _allItems[i, y] != null)
        ...
        for (int x = 0; x < _width; x++)
        {
            for (int y = 0; y < _height - 2; y++)
            {
                    ...
                    _allItems[x, y + 2]._itemType == current._itemType)
                {
                    ...
                    {
                        if (_board.IsActiveCell(x, i) && _allItems[x, i] != null)
        ...
        for (int x = 0; x < _width; x++)
        {
            ...
            for (int y = 0; y < _height; y++)
            {
                ...
                else if (emptySpaces > 0)
                {
                    ...
                    // Проверяем, что новая позиция активна
                    if (_board.IsActiveCell(x, newY))
                    {
                        _allItems[x, y] = null;
